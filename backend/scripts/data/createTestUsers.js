...
const createTestUsers = async () => {
  try {
    ...
    console.log(
      `🏢 Mevcut departmanlar: ${departments.map(d => d.ad).join(', ')}`,
    );

    // Eğer rol/departman yoksa oluştur
    if (roles.length === 0 || departments.length === 0) {
      console.log(
        '❌ Roller veya departmanlar eksik! Önce seed data çalıştırın.',
      );
      process.exit(1);
    }
    ...
    for (const userData of testUsers) {
      ...
      if (!userRole || !userDepartment) {
        console.log(
          `❌ ${userData.kullaniciAdi} için rol/departman bulunamadı, atlanıyor`,
        );
        continue;
      }
    ...
    const nonQcWorkers = workers.filter(user => {
      ...
      return (
        !roleName.includes('kalite kontrol') && !roleName.includes('admin')
      );
    });
