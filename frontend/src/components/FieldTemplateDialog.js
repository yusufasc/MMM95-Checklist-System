...
const FieldTemplateDialog = ({ open, onClose, fieldTemplate, categories, onSave }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (['select', 'multiselect'].includes(formData.tip) && formData.secenekler.length === 0) {
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Alan şablonu kaydedilirken hata oluştu');
    } finally {
      setLoading(false);
    }
  };
  ...
  const needsTextValidation = ['text', 'textarea', 'email', 'url', 'tel'].includes(formData.tip);
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              ...
              <Select value={formData.tip} onChange={handleChange('tip')} label="Alan Tipi">
                {fieldTypes.map(type => (
          ...
          <Grid item xs={12}>
            <Autocomplete
              ...
              renderInput={params => (
                <TextField {...params} label="Grup" variant="outlined" fullWidth />
              )}
            />
          </Grid>
          ...
          <Grid item xs={12} md={6}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', height: '100%' }}>
              <FormControlLabel
                control={<Switch checked={formData.gerekli} onChange={handleChange('gerekli')} />}
                label="Zorunlu Alan"
              />
              <FormControlLabel
                control={<Switch checked={formData.aktif} onChange={handleChange('aktif')} />}
                label="Aktif"
              />
            </Box>
          </Grid>
