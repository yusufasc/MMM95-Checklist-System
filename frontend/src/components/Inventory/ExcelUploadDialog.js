...
const ExcelUploadDialog = ({ open, onClose, categories, onUploadSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await inventoryAPI.importExcel(selectedCategory, file);
      ...
      if (error.response?.status === 400) {
        errorMessage =
          ...
          'Geçersiz dosya formatı veya kategori ID';
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const response =
        await inventoryAPI.downloadExcelTemplate(selectedCategory);
      ...
      link.setAttribute(
        'download',
        `envanter_template_${category?.ad || 'kategori'}.xlsx`,
      );
      ...
      {uploadResult && (
        <Box>
          <Alert severity='success' sx={{ mb: 2 }}>
            <strong>{uploadResult.basariliSayisi}</strong> kayıt başarıyla
            eklendi
            {uploadResult.toplamSatir && (
              <span> (Toplam {uploadResult.toplamSatir} satırdan)</span>
            )}
          </Alert>
                  ...
                  uploadResult.hatalar.map((error, index) => (
                    <ListItem key={`error-${index}`}>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          error.envanterKodu
                            ? `Envanter Kodu: ${error.envanterKodu}`
                            : ''
                        }
                      />
                    </ListItem>
                ...
                {uploadResult.uyarilar &&
                  uploadResult.uyarilar.map((warning, index) => (
                    <ListItem key={`warning-${index}`}>
                      ...
                      <ListItemText
                        primary={`Satır ${warning.row}: ${warning.message}`}
                      />
                    </ListItem>
                  ))}
